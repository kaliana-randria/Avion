...
public class ReservationVolServlet extends HttpServlet{
    ...
    //     AvionDAO avionDao = new AvionDAO();
        ...
        try {
            if ("eco".equals(action)) {
                Vols vol = dao.findById(idVol);
                req.setAttribute("volDetail", vol);
                String classeString = "Economique";
                req.setAttribute("classe", classeString);
                Classes classe = classeDAO.findByName(classeString);
    ...
    //             req.setAttribute("avion", avion);
            ...
            else if ("bus".equals(action)) {
                Vols vol = dao.findById(idVol);
                req.setAttribute("volDetail", vol);
                String classeString = "Business";
                req.setAttribute("classe", classeString);
                Classes classe = classeDAO.findByName(classeString);
                req.setAttribute("classeDonnees", classe);
                RequestDispatcher dispatcher = req.getRequestDispatcher("/WEB-INF/views/reservation.jsp");
